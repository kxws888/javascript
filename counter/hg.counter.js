    ...
    var Counter = Class({
        ...
        flush: function (value, time, rate) {
            ...
            timer = setInterval(function () {
                var num, last = Math.pow(10, self.length - i);
                if (delay || (+new Date() - start) > time) {
                    ...
                    num = Math.floor(Math.random() * Math.pow(10, self.length));
                }
                self.set(num);
            }, rate);
        },
...
/******************************************************************************************************************************************************************/
    ...
    hg.CounterTH = Class(Counter, {
        ...
        init: function (args) {
            ...
            this.bitHolder = document.querySelectorAll(args.bitHolder);

            for (var i = 0 ; i < this.length ; i += 1) {
                this.bits[i].html = this.bitHolder[i];
            }
        },
        ...
        set: function (value) {
            this.$super.set.call(this, value);
            for (var i = 0 ; i < this.length ; i += 1) {
                this.bits[i].html.innerHTML = this.bits[i].value;
            }
        }
    });
